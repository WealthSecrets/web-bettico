...
class MyPostsScreen extends GetWidget<ProfileController> {
  MyPostsScreen({Key? key, required this.userId, this.isOddboxes = false})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Obx(
      () => AppLoadingBox(
        ...
        child: (isOddboxes
                ? controller.myOddboxes.isEmpty
                : controller.myPosts.isEmpty)
            ? AppEmptyScreen(
                message: isOddboxes ? 'no_oddboxes'.tr : 'no_posts'.tr,
              )
            : ListView.builder(
                itemCount: isOddboxes
                    ? controller.myOddboxes.length
                    : controller.myPosts.length,
                itemBuilder: (BuildContext context, int index) {
                  final Post post = isOddboxes
                      ? controller.myOddboxes[index]
                      : controller.myPosts[index];
                  return TimelineCard(
                    post: post,
                    onTap: () {
