...
class WalletConnectQrCodeModal {
  ...
  Future<SessionStatus?> _createSessionWithModal(
    ...
    final CancelableCompleter<dynamic> cancelableCompleter =
        CancelableCompleter<dynamic>();
    final Completer<SessionStatus?> completer = Completer<SessionStatus?>();

    Future<SessionStatus?> createSession() async {
      try {
        final SessionStatus session = await _connector.createSession(
            chainId: chainId,
            onDisplayUri: (String uri) async {
              ...
              await showDialog<void>(
                  ...
                  builder: (BuildContext context) => ModalMainPage(
                        uri: uri,
                        walletCallback: (Wallet wallet) => _wallet = wallet,
                      ));
    ...
    await cancelableCompleter.operation.value.then((dynamic session) {
      sessionCreated = true;
      if (!isDismissed) {
        Navigator.of(context).pop();
      }
      if (!completer.isCompleted) {
        completer.complete(session as SessionStatus?);
      }
